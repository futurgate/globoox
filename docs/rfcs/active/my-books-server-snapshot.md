...
# RFC: ускорение `my-books` без потери консистентности

> Status: часть быстрых клиентских улучшений уже присутствует (`books` streaming, IndexedDB snapshots, SWR), но единый server-driven library snapshot из фазы 2 не реализован. Исходные trace JSON из этого документа отсутствуют в workspace; численные выводы ниже сохранены как историческое наблюдение, а не как воспроизводимый текущий benchmark.
...
1. `1 апреля`: исходный trace отсутствует.
2. `4 апреля`: исходный trace отсутствует.
3. `Версия на 9 апреля`: исходный trace отсутствует.
...
1. `src/app/(app)/my-books/page.tsx`
2. `src/lib/hooks/useAuth.ts`
3. `src/lib/useBooks.ts`
...
1. `src/lib/hooks/useSyncCheck.ts`
2. `src/components/SyncCheckClient.tsx`
...
1. `src/components/SyncCheckClient.tsx`
...
1. `server/api/books.get.ts` уже агрегировал список книг, effective visibility semantics и `selected_language`.
2. `server/api/sync/status.get.ts` отдавал sync version по scopes.
3. `server/api/books/[id]/reading-position.get.ts` возвращал reading position по одной книге с дополнительной валидацией anchor.

Эти backend-файлы находятся вне текущего workspace; утверждения требуют повторной проверки в backend-репозитории перед реализацией фазы 2.
...
1. `server/api/books.get.ts` (backend repository; not available in this workspace)
2. `server/api/sync/status.get.ts` (backend repository; not available in this workspace)
3. `server/api/books/[id]/reading-position.get.ts` (backend repository; not available in this workspace)
