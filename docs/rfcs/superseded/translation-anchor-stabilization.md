...
supersession_reason: undocumented-author-input-required
superseded_by: docs/decisions/ADR-0003-translation-v2-transitional-model.md
---

# RFC: Translation Anchor Stabilization via `translate-range`

> Historical outcome: the proposed `translate-range` route is not used by the current frontend. Translation v2 uses `/content` snapshot + `/blocks/text` reconcile + `/translate` push instead. The documents do not record why the “no adaptive batching”/one-shot-range approach was superseded; that rationale requires author confirmation.
...
New route:
- proposed `src/app/(app)/api/chapters/[id]/translate-range/route.ts`
...
4. `src/app/(app)/api/chapters/[id]/translate-range/route.ts` (proposed, never retained in current frontend)
5. `src/components/Reader/TranslationGlow.tsx` or related UI wrapper (if overlay hosted there)
6. Current replacement: `docs/reference/architecture/api-and-sync.md`
