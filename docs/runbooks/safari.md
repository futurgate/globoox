...
implementation:
  - src/components/Store/BookCard.tsx
---
...
**Legacy exception:** `src/lib/hooks/useEpubParser.ts` still contains an `img.onload -> drawImage` path. The hook currently has no call sites, but it must be fixed or removed before reuse.
