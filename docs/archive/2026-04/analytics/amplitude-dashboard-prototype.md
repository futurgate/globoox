...
type: archive
status: archived
disposition: provider-prototype-not-active
superseded_by: docs/reference/product/analytics-posthog.md
---

# Amplitude Analytics — Event Taxonomy & Dashboard Setup

> Historical provider prototype. Amplitude has no SDK or provider in the current repository. The original reason for choosing PostHog instead is not documented.
